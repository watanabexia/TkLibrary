...
def get_book(acc_number):
    session_new = DBSession()
    book = session_new.query(LibBooks).filter_by(
        Accession_Number=acc_number).one()
    session_new.close()
    return book
...
def get_book_BR(acc_number):
    ...
    * Returns None if no valid LibBook is found.
    """
    session_new = DBSession()
    book = session_new.query(Borrow_And_Return_Record).filter_by(
        Accession_Number=acc_number).one()
    session_new.close()
    return book
...
def is_book_on_loan(acc_number):
    try:
        session_new = DBSession()
        br_record = session_new.query(Borrow_And_Return_Record).filter_by(
            Accession_Number=acc_number, Return_Date=None).one()
...
def get_reserve_record(member_id, acc_number):
    session_new = DBSession()
    reserve_record = session_new.query(Reserve_Record).filter_by(
        Accession_Number=acc_number, memberid=member_id).one()
    session_new.close()
...
def get_book_Reserve(acc_number):
    ...
    * Returns None if no valid LibBook is found.
    """
    session_new = DBSession()
    book = session_new.query(Reserve_Record).filter_by(
        Accession_Number=acc_number).all()
    session_new.close()
    return book
...
def is_book_on_loan(acc_number):
    ...
    session_new = DBSession()
    try:
        session_new.query(Borrow_And_Return_Record).filter_by(
            Accession_Number=acc_number, Return_Date=None).one()
    except NoResultFound:
        session_new.close()
        return False
...
def is_book_reserved(acc_number):
    ...
    session_new = DBSession()
    try:
        session_new.query(Reserve_Record).filter_by(
            Accession_Number=acc_number).one()
    except NoResultFound:
        session_new.close()
        return False
...
def is_quota_reached(id):
    ...
    try:
        session_new = DBSession()
        session_new.query(LibMember).filter_by(
            memberid=id, current_books_borrowed=2).one()
...
def has_outstanding_fine(id):
    ...
    try:
        session_new = DBSession()
        session_new.query(LibMember).filter_by(
            memberid=id, outstanding_fee=0).one()
...
def get_Authors(acc_number):
    ...
    """
    res = ''
    books = session.query(Book_Author).filter_by(
        Accession_Number=acc_number).all()
    for book in books:
        res += book.Author + '\n'
    return res
...
def insert_reserve_record(member_id, acc_number, res_date):
    ...
    res_ins = res_ins.values(Accession_Number=acc_number,
                             memberid=member_id, Reserve_Date=res_date)
...
def update_member_reserved(member_id, reserved_number):
    session_new = DBSession()
    session_new.query(LibMember).filter_by(memberid=member_id).update(
        {'current_books_reserved': reserved_number})
    session_new.commit()
    session_new.close()
...
def delete_reserve_record(member_id, acc_number):
    session_new = DBSession()
    session_new.query(Reserve_Record).filter_by(
        memberid=member_id, Accession_Number=acc_number).delete()
    session_new.commit()
    session_new.close()
...
def insert_LibBooks(acc_number, Title, ISBN, Publisher, Year):
    ...
    book_ins = book_ins.values(Accession_Number=acc_number,
                               Title=Title, ISBN=ISBN, Publisher=Publisher, Year=Year)
...
def insert_Author(acc_number, AuthorList):
    for Author in AuthorList:
        ...
        author_ins = author_ins.values(
            Accession_Number=acc_number, Author=Author)
        conn.execute(author_ins)


def delete_All_Authors(acc_number):
    session_new = DBSession()
    session_new.query(Book_Author).filter_by(
        Accession_Number=acc_number).delete()
    session_new.commit()
    session_new.close()
...
def update_member_borrowed(member_id, borrowed_number):
    session_new = DBSession()
    session_new.query(LibMember).filter_by(memberid=member_id).update(
        {'current_books_borrowed': borrowed_number})
    session_new.commit()
    session_new.close()
...
def update_outstanding_fine(member_id, fine):
    session_new = DBSession()
    session_new.query(LibMember).filter_by(
        memberid=member_id).update({'outstanding_fee': fine})
    session_new.commit()
    session_new.close()
...
Mem_button = tk.Button(Root_frame, text="Memberships", width=20,
                       fg='black', command=lambda: change_frame(Root_frame, Mem_frame))
Mem_button.place(x=200, y=50, anchor="nw")
Book_button = tk.Button(Root_frame, text="Books", width=20,
                        fg='black', command=lambda: change_frame(Root_frame, Book_frame))
Book_button.place(x=200, y=100, anchor="nw")
Loan_button = tk.Button(Root_frame, text="Loans", width=20,
                        fg='black', command=lambda: change_frame(Root_frame, Loan_frame))
Loan_button.place(x=200, y=150, anchor="nw")
Res_button = tk.Button(Root_frame, text="Reservations", width=20,
                       fg='black', command=lambda: change_frame(Root_frame, Res_frame))
Res_button.place(x=200, y=200, anchor="nw")
Fine_button = tk.Button(Root_frame, text="Fines", width=20,
                        fg='black', command=lambda: change_frame(Root_frame, Fine_frame))
Fine_button.place(x=200, y=250, anchor="nw")
Rep_button = tk.Button(Root_frame, text="Reports", width=20,
                       fg='black', command=lambda: change_frame(Root_frame, Rep_frame))
...
top_text = tk.Label(
    Book_frame, text='Select One Of The Options Below', bg='cyan')
top_text.place(x=150, y=0, anchor="nw")

Acq_label = tk.Label(Book_frame, text="Book Acquisition", fg='black')
Acq_label.place(x=50, y=50, anchor="nw")
Acq_button = tk.Button(Book_frame, text="Acquire A Book", fg='black',
                       command=lambda: change_frame(Book_frame, Acq_frame))
...
Withd_button = tk.Button(Book_frame, text="Withdraw A Book",
                         fg='black', command=lambda: change_frame(Book_frame, Withd_frame))
Withd_button.place(x=300, y=100, anchor="nw")

Back_button = tk.Button(Book_frame, text="Back To Main Menu",
                        fg='black', command=lambda: change_frame(Book_frame, Root_frame))
Back_button.place(x=175, y=150, anchor="nw")


# Book Acquisition object

top_text_book_acquisition = tk.Label(
    Acq_frame, text='For New Book Acquisition, Please Enter Information Below', bg='cyan')
top_text_book_acquisition .place(x=50, y=0, anchor="nw")

Acc_number_label_book_acquisition = tk.Label(
    Acq_frame, text='Accession Number', fg='black')
...
Publisher_label_book_acquisition = tk.Label(
    Acq_frame, text='Publisher', fg='black')
...
def add_new_book():
    ...
    if book_exist(acc_number) or Title == "" or Author == "" or ISBN == "" or Publisher == "" or Year == "":
        messagebox.showinfo(
            title='Error!', message='Book Already Added; Duplicate, Missing or Incomplete fields')
    else:
        insert_LibBooks(acc_number, Title, ISBN, Publisher, Year)
        insert_Author(acc_number, Authorlist)
        # insert book inside LibBooks
        messagebox.showinfo(
            title='Success!', message='New Book Added In Library!')


Add_new_book_button_book_acquisition = tk.Button(
    Acq_frame, text="Add New Book", fg='black', command=add_new_book)
Add_new_book_button_book_acquisition.place(x=50, y=350, anchor="nw")


Back_to_book_button_book_acquisition = tk.Button(
    Acq_frame, text="Back To Book", fg='black', command=lambda: change_frame(Acq_frame, Book_frame))
...
def withdraw_book_on_loan_or_reserved(acc_number):
    if is_book_reserved(acc_number):
        messagebox.showinfo(
            title='Error!', message='Book Is Currently Reserved.')
    elif is_book_on_loan(acc_number):
        messagebox.showinfo(
            title='Error!', message='Book Is Currently On Loan.')
    else:
        delete_All_Authors(acc_number)
        delete_LibBooks(acc_number)
        messagebox.showinfo(
            title='Success!', message='Book Is Successfully Withdrawn.')


top_text_book_withdrawal = tk.Label(
    Withd_frame, text='To Remove Outdated Books From System, Please Enter Information Below', bg='cyan')
top_text_book_withdrawal.place(x=50, y=0, anchor="nw")

Acc_number_label_book_withdrawal = tk.Label(
    Withd_frame, text='Accession Number')
...
Withdraw_book_button_book_withdrawal = tk.Button(
    Withd_frame, text="Withdraw Book", fg='black', command=withdraw_book)
Withdraw_book_button_book_withdrawal.place(x=50, y=350, anchor="nw")
Back_to_book_button_book_withdrawal = tk.Button(
    Withd_frame, text="Back To Book", fg='black', command=lambda: change_frame(Withd_frame, Book_frame))
...
top_text = tk.Label(
    Loan_frame, text='Select One Of The Options Below', bg='cyan')
top_text.place(x=150, y=0, anchor="nw")

Borrow_label = tk.Label(Loan_frame, text="Book Borrow", fg='black')
Borrow_label.place(x=50, y=50, anchor="nw")
Borrow_button = tk.Button(Loan_frame, text="Borrow A Book", fg='black',
                          command=lambda: change_frame(Loan_frame, Borrow_frame))
...
Return_button = tk.Button(Loan_frame, text="Return A Book", fg='black',
                          command=lambda: change_frame(Loan_frame, Return_frame))
Return_button.place(x=300, y=100, anchor="nw")

Back_button = tk.Button(Loan_frame, text="Back To Main Menu",
                        fg='black', command=lambda: change_frame(Loan_frame, Root_frame))
Back_button.place(x=175, y=150, anchor="nw")


# Borrow object
top_text_book_borrow = tk.Label(
    Borrow_frame, text='To Borrow A Book , Please Enter Information Below', bg='cyan')
...
def borrow_book_on_loan_quota_fine(acc_number, member_id):
    ...
    if is_quota_reached(member_id):
        messagebox.showinfo(
            title='Error!', message='Member Loan Quota Exceeded.')
    elif is_book_on_loan(acc_number):
        book_due_date = get_due_date(acc_number, member_id)
        messagebox.showinfo(
            title='Error!', message='Book Is Currently On Loan Until ' + book_due_date)
    elif has_outstanding_fine(member_id):
        messagebox.showinfo(
            title='Error!', message='Member Has Outstanding Fines.')
    elif is_book_reserved(acc_number) and member_id not in members_reserved(acc_number):
        messagebox.showinfo(
            title='Error!', message='Book Is Already Reserved.')
    else:
        insert_borrow_and_return_record(
            acc_number, member_id, borrow_date, due_date())
        update_member_borrowed(member_id, new_borrowed_number)
        messagebox.showinfo(
            title='Success!', message='You Have Borrowed This Book.')
        # add 1 to books_borrowed and update borrow_and_return_record


Borrow_book_button_book_borrow = tk.Button(
    Borrow_frame, text="Borrow Book", fg='black', command=borrow_book)
Borrow_book_button_book_borrow.place(x=50, y=300, anchor="nw")
Back_to_loan_button_book_borrow = tk.Button(
    Borrow_frame, text="Back To Loan", fg='black', command=lambda: change_frame(Borrow_frame, Loan_frame))
Back_to_loan_button_book_borrow.place(x=700, y=300, anchor="nw")

# Return object

top_text_book_return = tk.Label(
    Return_frame, text='To Return A Book , Please Enter Information Below', bg='cyan')
...
def return_book_fine(member_id, acc_number, Fine):
    ...
    if Fine != 0:
        ...
        messagebox.showinfo(
            title='Error!', message='Book Returned Successfully But Member Has Fines.')
    else:
        update_member_borrowed(member_id, new_borrowed_number)
        delete_borrow_and_return_record(acc_number, member_id)
        messagebox.showinfo(
            title='Success!', message='You Have Returned This Book.')
        # minus 1 to books_borrowed and update borrow_and_return_record


Return_book_button = tk.Button(
    Return_frame, text="Return Book", fg='black', command=return_book)
Return_book_button.place(x=50, y=300, anchor="nw")
Back_to_loan_button = tk.Button(Return_frame, text="Back To Loan",
                                fg='black', command=lambda: change_frame(Return_frame, Loan_frame))
...
Res_book_button = tk.Button(Res_frame, text="Reserve a Book", fg='black',
                            command=lambda: change_frame(Res_frame, Res_book_frame))
Res_book_button.place(x=300, y=50, anchor="nw")
Res_cancel_label = tk.Label(
    Res_frame, text="Reservation Cancellation", fg='black')
Res_cancel_label.place(x=50, y=100, anchor="nw")
Res_cancel_button = tk.Button(Res_frame, text="Cancel Reservation",
                              fg='black', command=lambda: change_frame(Res_frame, Res_cancel_frame))
Res_cancel_button.place(x=300, y=100, anchor="nw")
Res_back_button = tk.Button(Res_frame, text="Back to Main Menu",
                            fg='black', command=lambda: change_frame(Res_frame, Root_frame))
Res_back_button.place(x=300, y=150, anchor="nw")

Res_book_title_label = tk.Label(
    Res_book_frame, text="To reserve a book, please enter information below:", fg='black')
Res_book_title_label.place(x=50, y=0, anchor="nw")
Res_book_Acc_number_label = tk.Label(
    Res_book_frame, text="Accession Number", fg='black')
Res_book_Acc_number_label.place(x=50, y=50, anchor="nw")

Res_book_Acc_number_entry = tk.Entry(
    Res_book_frame, fg='black', bg='white', width=60)
...
Res_book_Mem_ID_label = tk.Label(
    Res_book_frame, text="Membership ID", fg='black')
Res_book_Mem_ID_label.place(x=50, y=100, anchor="nw")
Res_book_Mem_ID_entry = tk.Entry(
    Res_book_frame, fg='black', bg='white', width=60)
...
Res_book_Res_date_label = tk.Label(
    Res_book_frame, text="Reserve date (DD/MM/YYYY)", fg='black')
Res_book_Res_date_label.place(x=50, y=150, anchor="nw")
Res_book_Res_date_entry = tk.Entry(
    Res_book_frame, fg='black', bg='white', width=60)
    ...
    if (is_book_on_loan(acc_number)):
        ...
        try:
            ...
            messagebox.showerror(
                title="Error", message="\"{}\" has already reserved the book \"{}\".".format(mem.name, book.Title))
            return
        ...
        if (mem.outstanding_fee == 0):
            ...
            if (mem.current_books_reserved < 2):
                ...
                messagebox.showerror(
                    title="Error", message="\"{}\" has already reserved 2 books. No more reservation is allowed.".format(mem.name))
                return
        else:
            messagebox.showerror(title="Error", message="\"{}\" has unpaid outstanding fine of {}. Please pay before any reservation.".format(
                mem.name, mem.outstanding_fee))
            return
    else:
        messagebox.showerror(
            title="Error", message="\"{}\" is available. You may go ahead and borrow it now.".format(book.Title))
        return

    messagebox.showinfo(
        title="Success", message="\"{}\" have successfully reserved the book \"{}\".".format(mem.name, book.Title))
...
def confirm_book_reservation():
    ...
    try:
        ...
        messagebox.showerror(
            title="Error", message="\"{}\" is not a valid date or a valid date format.".format(date_string))
        return
    ...
    try:
        ...
        messagebox.showerror(
            title="Error", message="\"{}\" is not a valid member id.".format(mem_id))
        return
    ...
    try:
        ...
        messagebox.showerror(
            title="Error", message="\"{}\" is not a valid accession number.".format(acc_number))
        return
    ...
    Res_book_confirm_label = tk.Label(
        Res_book_confirm_top, text="Confirm Reservation Details To Be Correct", fg='black')
    Res_book_confirm_label.place(x=50, y=0, anchor="nw")
    Res_book_confirm_acc_label = tk.Label(
        Res_book_confirm_top, text="Accession Number: {}".format(acc_number), fg='black')
    Res_book_confirm_acc_label.place(x=50, y=50, anchor="nw")
    Res_book_confirm_title_label = tk.Label(
        Res_book_confirm_top, text="Book Title: {}".format(book.Title), fg='black')
    Res_book_confirm_title_label.place(x=50, y=100, anchor="nw")
    Res_book_confirm_memid_label = tk.Label(
        Res_book_confirm_top, text="Membership ID: {}".format(mem_id), fg='black')
    Res_book_confirm_memid_label.place(x=50, y=150, anchor="nw")
    Res_book_confirm_name_label = tk.Label(
        Res_book_confirm_top, text="Member Name: {}".format(mem.name), fg='black')
    Res_book_confirm_name_label.place(x=50, y=200, anchor="nw")
    Res_book_confirm_date_label = tk.Label(
        Res_book_confirm_top, text="Reserve Date (DD/MM/YYYY): {}".format(date_string), fg='black')
    Res_book_confirm_date_label.place(x=50, y=250, anchor="nw")
    Res_book_confirm_button = tk.Button(Res_book_confirm_top, text="Confirm Reservation", fg='black',
                                        command=lambda: commit_book_reservation(mem, book, res_date, Res_book_confirm_top))
    Res_book_confirm_button.place(x=50, y=300, anchor="nw")

    Res_book_confirm_top.loop()


Res_book_Res_button = tk.Button(
    Res_book_frame, text="Reserve Book", fg='black', command=confirm_book_reservation)
Res_book_Res_button.place(x=50, y=200, anchor="nw")
Res_book_Back_button = tk.Button(Res_book_frame, text="Back to Reservation Menu",
                                 fg='black', command=lambda: change_frame(Res_book_frame, Res_frame))
Res_book_Back_button.place(x=700, y=200, anchor="nw")

Res_cancel_title_label = tk.Label(
    Res_cancel_frame, text="To cancel a reservation, please enter information below:", fg='black')
Res_cancel_title_label.place(x=50, y=0, anchor="nw")
Res_cancel_Acc_number_label = tk.Label(
    Res_cancel_frame, text="Accession Number", fg='black')
Res_cancel_Acc_number_label.place(x=50, y=50, anchor="nw")
Res_cancel_Acc_number_entry = tk.Entry(
    Res_cancel_frame, fg='black', bg='white', width=60)
...
Res_cancel_Mem_ID_label = tk.Label(
    Res_cancel_frame, text="Membership ID", fg='black')
Res_cancel_Mem_ID_label.place(x=50, y=100, anchor="nw")
Res_cancel_Mem_ID_entry = tk.Entry(
    Res_cancel_frame, fg='black', bg='white', width=60)
...
def commit_cancel_reservation(mem, book, toplevel):
    ...
    try:
        ...
        messagebox.showerror(
            title="Error", message="\"{}\" hasn't reserved the book \"{}\" yet.".format(mem.name, book.Title))
        return
    else:
    ...
    messagebox.showinfo(
        title="Success", message="\"{}\" have successfully cancelled the reservation for the book \"{}\".".format(mem.name, book.Title))
...
def confirm_cancel_reservation():
    ...
    Res_cancel_confirm_label = tk.Label(
        Res_cancel_confirm_top, text="Confirm Cancellation Details To Be Correct", fg='black')
    Res_cancel_confirm_label.place(x=50, y=0, anchor="nw")
    Res_cancel_confirm_acc_label = tk.Label(
        Res_cancel_confirm_top, text="Accession Number: {}".format(acc_number), fg='black')
    Res_cancel_confirm_acc_label.place(x=50, y=50, anchor="nw")
    Res_cancel_confirm_title_label = tk.Label(
        Res_cancel_confirm_top, text="Book Title: {}".format(book.Title), fg='black')
    Res_cancel_confirm_title_label.place(x=50, y=100, anchor="nw")
    Res_cancel_confirm_memid_label = tk.Label(
        Res_cancel_confirm_top, text="Membership ID: {}".format(mem_id), fg='black')
    Res_cancel_confirm_memid_label.place(x=50, y=150, anchor="nw")
    Res_cancel_confirm_name_label = tk.Label(
        Res_cancel_confirm_top, text="Member Name: {}".format(mem.name), fg='black')
    Res_cancel_confirm_name_label.place(x=50, y=200, anchor="nw")
    Res_cancel_confirm_button = tk.Button(Res_cancel_confirm_top, text="Confirm Cancellation",
                                          fg='black', command=lambda: commit_cancel_reservation(mem, book, Res_cancel_confirm_top))
    Res_cancel_confirm_button.place(x=50, y=300, anchor="nw")

    Res_cancel_confirm_top.loop()


Res_cancel_Res_button = tk.Button(
    Res_cancel_frame, text="Cancel Reservation", fg='black', command=confirm_cancel_reservation)
Res_cancel_Res_button.place(x=50, y=200, anchor="nw")
Res_cancel_Back_button = tk.Button(Res_cancel_frame, text="Back to Reservation Menu",
                                   fg='black', command=lambda: change_frame(Res_cancel_frame, Res_frame))
...
Books_on_Loan_to_Member__results_frame = tk.Frame(
    root, height=win_h, width=win_w)

# report frame Main menu
top_text = tk.Label(
    Rep_frame, text='Select One Of The Options Below', bg='cyan')
top_text.place(x=50, y=0, anchor="nw")

Book_Search_label = tk.Label(
    Rep_frame, text="A member can perform a search \n on the collection of books.", fg='black')
Book_Search_label.place(x=400, y=50, anchor="nw")
Book_Search_button = tk.Button(Rep_frame, text="Book Search", width=25,
                               fg='black', command=lambda: change_frame(Rep_frame, Book_search_frame))
Book_Search_button.place(x=50, y=50, anchor="nw")

Book_on_Loan_label = tk.Label(
    Rep_frame, text="This function displays all the books \n currently on loan to members.", fg='black')
Book_on_Loan_label.place(x=400, y=120, anchor="nw")
Book_on_Loan_button = tk.Button(Rep_frame, text="Books on Loan", width=25,
                                fg='black', command=lambda: change_frame(Rep_frame, Book_on_Loan_frame))
Book_on_Loan_button.place(x=50, y=120, anchor="nw")

Book_on_reservation_label = tk.Label(
    Rep_frame, text="This function displays all the books \n that members have reserved.", fg='black')
Book_on_reservation_label.place(x=400, y=190, anchor="nw")
Book_on_reservation_button = tk.Button(Rep_frame, text="Books on Reservation", width=25,
                                       fg='black', command=lambda: change_frame(Rep_frame, Book_on_reservation_frame))
Book_on_reservation_button.place(x=50, y=190, anchor="nw")

Outstanding_Fines__label = tk.Label(
    Rep_frame, text="This function displays the \n outstanding fines for members.", fg='black')
Outstanding_Fines__label.place(x=400, y=260, anchor="nw")
Outstanding_Fines__button = tk.Button(Rep_frame, text="Outstanding Fines", width=25,
                                      fg='black', command=lambda: change_frame(Rep_frame, Outstanding_Fines__frame))
Outstanding_Fines__button.place(x=50, y=260, anchor="nw")

Books_on_Loan_to_Member__label = tk.Label(
    Rep_frame, text="This function displays all the books a member \n identified by the membership ID has borrowed.", fg='black')
Books_on_Loan_to_Member__label.place(x=400, y=330, anchor="nw")
Books_on_Loan_to_Member__button = tk.Button(Rep_frame, text="Books on Loan to Member", width=25,
                                            fg='black', command=lambda: change_frame(Rep_frame, Books_on_Loan_to_Member__frame))
Books_on_Loan_to_Member__button.place(x=50, y=330, anchor="nw")

Back_button = tk.Button(Rep_frame, text="Back To Main Menu",
                        fg='black', command=lambda: change_frame(Rep_frame, Root_frame))
Back_button.place(x=300, y=400, anchor="nw")

# Book search frame
top_text = tk.Label(
    Book_search_frame, text='Select based on one of the categories below:', bg='cyan')
top_text.place(x=50, y=0, anchor="nw")
...
Search_book_buttom = tk.Button(Book_search_frame, text="Search Book", width=20, height=1,
                               command=lambda: change_frame(Book_search_frame, Book_search_results_frame))
Search_book_buttom.place(x=50, y=300, anchor="nw")

Back_to_Report_Main = tk.Button(Book_search_frame, text="Back to Reports Menu",
                                width=20, height=1, command=lambda: change_frame(Book_search_frame, Rep_frame))
Back_to_Report_Main.place(x=700, y=300, anchor="nw")

# Book Search Results frame
# top word
top_text = tk.Label(Book_search_results_frame,
                    text='Book Search Results', bg='cyan')
top_text.place(x=50, y=0, anchor="nw")

Back_to_Search_Function = tk.Button(Book_search_results_frame, text="Back To Search Function",
                                    width=20, height=1, command=lambda: change_frame(Book_search_results_frame, Book_search_frame))
...
Back_to_Report_Main = tk.Button(Book_on_Loan_frame, text="Back to Reports Menu",
                                width=20, height=1, command=lambda: change_frame(Book_on_Loan_frame, Rep_frame))
Back_to_Report_Main.place(x=50, y=100, anchor="nw")

# Book on reservation frame
top_text = tk.Label(Book_on_reservation_frame,
                    text='Books on Reservation Report', bg='cyan')
top_text.place(x=50, y=0, anchor="nw")

Back_to_Report_Main = tk.Button(Book_on_reservation_frame, text="Back to Reports Menu",
                                width=20, height=1, command=lambda: change_frame(Book_on_reservation_frame, Rep_frame))
Back_to_Report_Main.place(x=50, y=100, anchor="nw")


# Outstanding Fines frame
top_text = tk.Label(Outstanding_Fines__frame,
                    text='Members With Outstanding Fines', bg='cyan')
top_text.place(x=50, y=0, anchor="nw")

Back_to_Report_Main = tk.Button(Outstanding_Fines__frame, text="Back to Reports Menu",
                                width=20, height=1, command=lambda: change_frame(Outstanding_Fines__frame, Rep_frame))
Back_to_Report_Main.place(x=50, y=100, anchor="nw")


# Books on Loan to Member search frame
top_text = tk.Label(Books_on_Loan_to_Member__frame,
                    text='Books on Loan to Member', bg='cyan')
top_text.place(x=50, y=0, anchor="nw")
...
Search_mem = tk.Button(Books_on_Loan_to_Member__frame, text="Search Member", fg='black', command=lambda: change_frame(
    Books_on_Loan_to_Member__frame, Books_on_Loan_to_Member__results_frame))
...
Back_to_Report_Main = tk.Button(Books_on_Loan_to_Member__frame, text="Back to Reports Menu",
                                fg='black', command=lambda: change_frame(Books_on_Loan_to_Member__frame, Rep_frame))
Back_to_Report_Main.place(x=700, y=350, anchor="nw")

# Books on Loan to Member results frame
top_text = tk.Label(Books_on_Loan_to_Member__results_frame,
                    text='Books on Loan to Member', bg='cyan')
top_text.place(x=50, y=0, anchor="nw")

Back_to_Report_Main = tk.Button(Books_on_Loan_to_Member__results_frame, text="Back to Reports Menu",
                                width=20, height=1, command=lambda: change_frame(Books_on_Loan_to_Member__results_frame, Rep_frame))
Back_to_Report_Main.place(x=50, y=100, anchor="nw")
